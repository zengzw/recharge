...
import com.tsh.traintickets.vo.request.RefundTicketCallbackRequest;
...
public class RefundTicketCallBackController extends BaseController {

    @Autowired
    KuyouService kuyouService;

    @RequestMapping(value = "/dealRefund.do", method = RequestMethod.POST )
    @ResponseBody
    public void dealRefundCallBack() {
        ...
        try {
            ...
            if(StringUtils.isNotEmpty(timestamp) && StringUtils.isNotEmpty(jsonparams) &&
                ...
                RefundTicketCallbackRequest callbackRequest = JsonUtils.convert2Object(jsonparams, RefundTicketCallbackRequest.class);
                logger.info("-------> refundCallbackRequest: " + JSON.toJSONString(callbackRequest));
                kuyouService.dealRefundTicketCallBack(ConvertRequest2BO.convertRefundTicketCallBack(callbackRequest));
            }
