 ...
 */
@Service("verifyUsersHandler")
public class VerifyUsersHandler extends AbstractKuyouHttpHandler implements TicketHttpHandler {

    private static Logger logger = LogManager.getLogger(VerifyUsersHandler.class);
    ...
    public String doHandler(Object request){
        ...
        VerifyUsersSupplierRequest supplierRequest = (VerifyUsersSupplierRequest) request;

        // 加密字符串body
        StringBuilder paramsBuilder = new StringBuilder();
        paramsBuilder.append(super.getHeaderParamsString(timestamp)).
                append(JSON.toJSONString(supplierRequest));

        paramsMap.put("json_param", JSON.toJSONString(supplierRequest));

        // 设置加密内容
        paramsMap.put("hmac", getEncrypt(paramsBuilder.toString()));

        String result = null;
        logger.info("------------------> verifyUsers request:" + JSON.toJSONString(paramsMap));
        try{
            result = HttpXmlClient.post(DynamicParamsUtils.getParam(DynamicParamsEnums.kuyou_url), paramsMap);
        } catch (Exception e){
            logger.error("------------------> verifyUsers error:"+e.getMessage(), e);
            throw new BizException(ResponseCode.VERIFY_USERS_ERROR);
        }
        result = super.getUtf8(result);
        logger.info("------------------> verifyUsers response:" + result);
    ...
    protected String getMethodName() {
        return KuyouSupplierMethod.verifyUsers.toString();
    }
}
